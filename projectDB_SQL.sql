...
-- * Generation date: Wed Jan 21 16:02:30 2026 
-- * LUN file: C:\Users\Utente\Desktop\progettoDB\projectDB.lun 
-- * Schema: schema_relazionale/1 
-- ********************************************* 
...
create database schema_relazionale;
use schema_relazionale;
...
create table SUPERVISIONE_G (
     nomeGiostra varchar(20) not null,
     CF varchar(20) not null,
     constraint ID_SUPERVISIONE_G_ID primary key (nomeGiostra, CF));

create table SUPERVISIONE_RUOTA (
     CF varchar(20) not null,
     nomeRuota varchar(20) not null,
     constraint ID_SUPERVISIONE_RUOTA_ID primary key (nomeRuota, CF));
...
-- Not implemented
-- alter table AREA_TEMATICA add constraint ID_AREA_TEMATICA_CHK
--     check(exists(select * from LAVORATORE
--                  where LAVORATORE.nomeAreaTematica = nomeAreaTematica)); 

-- Not implemented
-- alter table AREA_TEMATICA add constraint ID_AREA_TEMATICA_CHK
--     check(exists(select * from SUPERVISIONE_AT
--                  where SUPERVISIONE_AT.nomeAreaTematica = nomeAreaTematica)); 
...
-- Not implemented
-- alter table ATTRAZIONE_DI_PAURA add constraint ID_ATTRAZIONE_DI_PAURA_CHK
--     check(exists(select * from SUPERVISIONE_AP
--                  where SUPERVISIONE_AP.nomeAttrazionePaura = nomeAttrazionePaura)); 
...
-- Not implemented
-- alter table GIOSTRA add constraint ID_GIOSTRA_CHK
--     check(exists(select * from SUPERVISIONE_G
--                  where SUPERVISIONE_G.nomeGiostra = nomeGiostra)); 
...
-- Not implemented
-- alter table RUOTA_PANORAMICA add constraint ID_RUOTA_PANORAMICA_CHK
--     check(exists(select * from SUPERVISIONE_RUOTA
--                  where SUPERVISIONE_RUOTA.nomeRuota = nomeRuota)); 
...
alter table SCONTO add constraint FKAPPLICAZIONE
     foreign key (nomeBiglietto)
     references TIPOLOGIA_BIGLIETTO (nomeBiglietto);
...
create unique index ID_SUPERVISIONE_AP_IND
     on SUPERVISIONE_AP (nomeAttrazionePaura, CF);

create index FKSUP_LAV_3_IND
...
create unique index ID_SUPERVISIONE_G_IND
     on SUPERVISIONE_G (nomeGiostra, CF);

create index FKSUP_LAV_1_IND
     on SUPERVISIONE_G (CF);

create unique index ID_SUPERVISIONE_RUOTA_IND
     on SUPERVISIONE_RUOTA (nomeRuota, CF);

create index FKSUP_LAV_IND
     on SUPERVISIONE_RUOTA (CF);
